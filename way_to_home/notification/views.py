...
class NotificationView(View):
    ...
    def get(self, request, way_id, notification_id=None):
        ...
        user = request.user
        way = user.ways.filter(id=way_id)

        if not way:
            return HttpResponse('way is not found', status=400)

        if not notification_id:
            data = [notification.to_dict() for notification in way[0].notifications.all().order_by('week_day')]
            return JsonResponse(data, status=200, safe=False)

        notification = way[0].notifications.filter(id=notification_id)
        if not notification:
            return HttpResponse('Notification not found', status=400)

        return JsonResponse(notification[0].to_dict(), status=200)
    ...
    def put(self, request, way_id, notification_id=None):  # pylint: disable=R0201
        ...
        is_updated = notification[0].update(**data)
        if not is_updated:
